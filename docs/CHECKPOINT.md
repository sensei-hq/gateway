...
measured renderer, and `ContextBudgeted` folded FIRST-wins. `cdea80d`+`16a344e` T5/T6 `drive_agent`
...
`AgentRun.context_cut`, and executor-level proof the truncation MARKER reached the provider.
...
reported a degraded node as un-degraded when read back. `project_agent_outputs` now re-attaches the
key from the durable `ContextBudgeted` row (`effect_id(node, 0, 0)`) — no new event.
...
0 · `fmt --check` 0. The read-back test was RED first on the stated reason; its own premise guard
was mutation-verified (disabling the terminal short-circuit grows the journal by one
`RunCompleted`, AFTER the zero-call assertion passes — so the call count was never the guard). The
two green-on-arrival guards were mutation-verified: `Some(c)` -> `None` on the replay arm, and a
...
Nine plan errors and SEVEN false doc claims. `03204bf` corrected two more of its own slice's:
`finish_agent`'s "exactly ONE place the canonical output is built" (there are two, and the second
drifted) and the unasserted claim that the key rides every drive. It also caught a false claim in
its OWN first draft before commit ("an empty script makes a re-drive fail loudly" — a re-drive
replays from memo and calls nothing). **Sensei daemon NOT running — this file is the only record.**
