...
- **SP-6 s2 ✅ tasks 1–8** — `GateOption`/`GateOutcome` + `GateAwaited`/`GateDecided` (new variants
  ...
  split into shared `gate_precheck`/`wait_or_expire`/`pause_awaiting` · the gate fold (decisions
...
- **Re-review OF the review fixes ✅** — 7 findings, three commits: 3 HIGH in `gate decide`'s
  post-append classification (a honoured `reject` reported "not read"; both "not read" arms
  asserted by nothing — the collapse left 192 green; a journal fault echoed raw, fixed in
  `cmd::run::signal` too) · 2 MED bounds (`--as`, the success line) · 2 MED docs.
- **SP-6 s1 · SP-DATA 1–5 · SP-3 · SP-4 ✅** on `main` via PR #46.
