...
`2026-09-04-sp-7b-context-budgeting-design.md` (12 ACs) + its plan. SP-7a DONE (`864a8dd`).
...
read back a degraded node as un-degraded. `project_agent_outputs` now re-attaches it from the
durable `ContextBudgeted` row (`effect_id(node, 0, 0)`) — no new event.
...
T8: judge whether `oversized_dependency_context_halts_over_budget_never_truncates` still reads right
(its resize is MOOT — T5 established the fixture already halts at the floor), then sweep
`PromptParts::join`'s and `render_context_section_bounded`'s docs, which still argue the model path
must NEVER truncate. Nine plan errors, SEVEN false doc claims; `03204bf` corrected two more of its
own slice's (`finish_agent`'s "exactly ONE place the canonical output is built" — there are two —
and the unasserted claim that the key rides every drive) and caught one in its OWN draft first.
