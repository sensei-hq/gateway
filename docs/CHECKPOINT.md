...
- **Tasks 1–5** — `GateSpec::Human`/`LoopGateOption`; `validate_dag` rejects a menu that
  cannot converge, at every depth; the two journal variants; the fold (ask FIRST-wins into
  the SHARED `deadlines` map, decision LAST-wins); the `human_question_for` seam.
...
Nothing. Open question in the plan, NOT a blocker: `validate_dag` does not reserve the
`__gate__` segment, so an authored one in a `Loop` body collides with the gate path.
