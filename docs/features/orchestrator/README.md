...
- **Persistence (SP-DATA):** durable run state (journal + CAS + context in Postgres) landed in SP-DATA-1 — see the [overview decision log](../../superpowers/orchestrator-overview.md#3-decision-log-asked--confirmed). `PostgresConfigSource`, durable `config_versions`, and the durable scheduler are the remaining SP-DATA slices.
