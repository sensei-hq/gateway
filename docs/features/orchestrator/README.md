...
| [Execution graph](execution-graph.md) | Partial (Phase 3 · SP-1/3 · SP-6-1) | `crates/orchestrator*` | `ModelCall`/`Agent`/`Map`/`Consolidate`/`Loop` · typed edges + `validate_dag` · ready-node scheduler · aggregation · gated iteration (deterministic gate, `converged` finalize, resume-replay) · **`AwaitSignal`** (SP-6 s1 — the HITL primitive: pause until a signal arrives, answer journaled + folded so a resume never re-asks, optional **journaled absolute** deadline that FAILS the node) · `Subgraph`/`Branch`/`PlanDelta` deferred |
| [Durable journal](durable-journal.md) | Partial (SP-1 · SP-DATA-1 · SP-DATA-5 · SP-6-1) | `orchestrator-core` · `orchestrator-store` | effect classes · two-phase + in-doubt · replay/resume · **durable Postgres backend** (`PostgresJournal`/`ContentStore`/`ContextStore`, `postgres` feature) — cross-process resume, zero re-spend, `format_version` fence; feature-off byte-identical · **durable spend ledger** (`EffectRecorded{usage}`/`RunStarted{budget}`/`BudgetRaised`, folded by effect id) · **HITL signals** (`SignalAwaited` folded FIRST-wins, `SignalReceived` LAST-wins — new variants, so `FORMAT_VERSION` stays 1) |
