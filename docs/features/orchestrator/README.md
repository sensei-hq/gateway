...
- **Persistence (SP-DATA):** durable run state (journal + CAS + context in Postgres) landed in SP-DATA-1; **`PostgresConfigSource` + durable `config_versions`/`bump_config_version` (the cross-process config-version fence)** landed in SP-DATA-2 — see the [overview decision log](../../superpowers/orchestrator-overview.md#3-decision-log-asked--confirmed). The durable scheduler, management CLI/API, and cost/token budget model are the remaining SP-DATA slices.
