...
4. **Full CAS** — journal/payload split + size threshold + content-addressing, **plus** snapshots (resume = latest snapshot + replay-the-tail — *the resume half is **unwired**; snapshots are write-only, see §5.2's amendment*) and compaction (terminal `Map` children → `{status,digest,cost}` — *`ModelCall`-body Maps only, and the shipped record is `{index,status,digest,input_hash,usage}`; see §5.3's amendment*).
...
**In slice 3:**
- **Execution model:** a **DAG scheduler** (ready-node dispatch under a bounded global concurrency cap — *as shipped: **round-based** ready-node dispatch, **sequential within a round**; the bounded cap applies to `Map` children only, see §3.2's amendment*) replacing the in-order linear drive; `NodeKind::{Map, Consolidate}`; typed `Dep`/`EdgeKind`; `validate_dag`; cascade-skip across hard edges (`NodeSkipped`).
...
- The executor dispatches all ready nodes concurrently, capped by `Executor.concurrency` (default 8; a `tokio::sync::Semaphore`). After a wave completes it re-computes ready nodes (a **round**), writes a snapshot (§5.2), and continues until no nodes remain.
  - ⚠️ **SUPERSEDED — the ready set does not run concurrently.** As written: "The executor dispatches all ready nodes concurrently, capped by `Executor.concurrency` (default 8; a `tokio::sync::Semaphore`)."
  - **What shipped: the ready set runs SEQUENTIALLY, one node at a time.** `Executor::drive` (`crates/orchestrator/src/executor/mod.rs`) is `let ready = ready_nodes(..); if ready.is_empty() { break; } for node in ready { let result = self.run_node(..).await?; self.apply_node_result(..).await?; }` — one `.await` per node, in `graph.nodes` declaration order, with no spawn and no join across the DAG. Two independent ready branches therefore run one after the other, deterministically. (`ExpansionCounters`' doc comment in the same file relies on this, calling it "the sequential top-level drive loop" to justify `Relaxed` ordering.)
  - **`Executor.concurrency` is a `Map`-child cap, not a scheduler cap.** Default 8 (`Executor::new`), overridden by `with_concurrency`, and documented there as "the ceiling on how many `Map` children run at once (bounded by `min(map.concurrency, executor.concurrency)`)". The only `tokio::sync::Semaphore` in the executor is the one the `Map` fan-out builds from that `min` (`crates/orchestrator/src/executor/fanout.rs`). Concurrency lives INSIDE a `Map`, not across the DAG.
  - The rest of the bullet holds unchanged: after the round completes the executor re-computes the ready set, writes a snapshot (§5.2), and continues until no nodes remain.
...
- After each **round** (§3.2) the executor writes `SnapshotWritten{seq}` + a `Snapshot { completed, skipped, memo_digests, context_refs, map_manifests }` to the store (keyed by `RunId`, latest wins).
  - ⚠️ **SUPERSEDED — there is no `SnapshotWritten` event, and the payload's field list is not the shipped one.** As written: "the executor writes `SnapshotWritten{seq}` + a `Snapshot { completed, skipped, memo_digests, context_refs, map_manifests }`".
  - **A snapshot is written OUT-OF-BAND, with no journal event at all.** `SnapshotWritten` was never added to `JournalEvent`: it appears nowhere in `crates/` — the name occurs only in docs (this bullet, §2's table, and the doc-truth plan that flagged the drift). `Executor::write_snapshot` (`crates/orchestrator/src/executor/durability.rs`) calls `journal.snapshot(run, snap)` directly — deliberately, so the control-flow event order stays byte-identical to slice-1/2 (a backend without snapshot support no-ops via the trait default). "Keyed by `RunId`, latest wins" holds.
  - **The shipped payload is `Snapshot { seq, completed, skipped, outputs, spent, budget }`** (`crates/orchestrator-core/src/journal.rs`): `outputs` is each completed node's output as a ref-or-inline `EffectOutput`, and `spent`/`budget` are SP-DATA-5's two ledger scalars. `memo_digests`, `context_refs` and `map_manifests` are NOT carried — the per-effect memo, the blackboard and each Map's manifest are rebuilt by folding the journal instead.
- **Resume** = load the latest `Snapshot` + replay only the journal tail (events with `Seq >` the snapshot's) → bounds fold cost for wide/long runs. A run with no snapshot folds from the start (slice-1/2 path).
  - ⚠️ **SUPERSEDED — snapshot-seeded resume was never wired. Snapshots are write-only today.** As written: "**Resume** = load the latest `Snapshot` + replay only the journal tail (events with `Seq >` the snapshot's)."
  - **Every resume folds the WHOLE journal** — i.e. always the "folds from the start" path this bullet reserved for a run with no snapshot. `Executor::start_inner` (`crates/orchestrator/src/executor/mod.rs`) does `self.journal.load(run)` and then `fold_journal(&events)`; it never calls `latest_snapshot`. `ExecutionJournal::latest_snapshot` (declared in `crates/orchestrator-core/src/journal.rs`, implemented in `crates/orchestrator-store/src/lib.rs` and `postgres.rs`) has **no non-test caller** in the workspace, and `load_since`'s only production caller is `Scheduler::earliest_resume_after` (`crates/orchestrator/src/scheduler.rs`), which scans one drive's `RunPaused` deadlines and folds no run state.
  - **The write side is real, so the optimisation stays available:** `write_snapshot` runs at every round boundary. Whoever wires the read side inherits `Snapshot.spent`/`Snapshot.budget`, added by SP-DATA-5 as the two scalars a tail-only fold cannot re-derive — see the doc comment on `Snapshot::spent`, which spells out that a seeded resume without them silently uncaps the budget gate.
...
- Once a `Map`'s children are all terminal **and** its `Consolidate` is `Completed`, the per-child journal records collapse to `{ index, status, digest }` — the full child transcript remains retrievable from CAS by digest but leaves the hot fold path. Compaction is journaled (auditable) and **never** drops a digest (content stays addressable). *(Per-child `cost` in the compacted record — master §7.4 — is deferred: the orchestrator has no cost plumbing from the gateway's `estimated_cost`/`actual_cost` yet; adding it is a later, additive change.)*
  - ⚠️ **AMENDED — this holds for a `ModelCall`-body `Map` only, and the record carries more than three fields.** As written the sentence covers every `Map`, unqualified.
  - **An `Agent`-body `Map` is never compacted.** The sole call site is `apply_node_result`'s `if let Some(over) = consolidate_compaction_target(graph, node) { self.compact_map(..) }` (`crates/orchestrator/src/executor/mod.rs`), and `consolidate_compaction_target` (`crates/orchestrator/src/executor/support.rs`) returns `None` unless the `Consolidate`'s `over` node is a `Map` whose body is `MapBody::ModelCall` — its doc says "`Agent`-body Maps are multi-effect and are not compacted". An Agent child is a whole ReAct sub-run of several effects, so its per-child records stay in the hot fold path.
  - **Compaction also requires a configured `ContentStore`:** `compact_map` (`crates/orchestrator/src/executor/durability.rs`) opens with `let Some(content) = &self.content else { return Ok(()) };` — a no-CAS executor skips it, there being nowhere to keep the content addressable.
  - **The compacted record is `CompactChild { index, status, digest, input_hash, usage }`** (`crates/orchestrator-core/src/journal.rs`), appended as `MapCompacted { node, children }`. `input_hash` is what lets the resume fold rebuild each child's memo entry as a content ref; `usage` is the child's `TokenUsage`, moved onto the manifest by SP-DATA-5 because the record holding it is being deleted. The deferred `cost` parenthetical still stands — `usage` is tokens, and `estimated_cost`/`actual_cost` still appear nowhere in `crates/orchestrator*`.
...
- **Resume across fan-out:** folding (from the latest snapshot — *unwired; the fold is over the FULL journal, see §5.2's amendment*) rebuilds completed/skipped sets, the memo, the context store, and each `Map`'s child manifest; **completed children/agents memoize with zero re-spend** (the slice-1/2 headline extends across fan-out). A partial `Map` re-dispatches only its incomplete children.
...
8. **Snapshot resume (headline)** — a run that dies mid-fan-out resumes from the latest snapshot + tail and **re-spends nothing** for completed children (assert each completed child's effect-id appears in exactly one `EffectRecorded`, and the run-2 gateway is called only for the unfinished children). — ⚠️ **Amended:** "from the latest snapshot + tail" did not ship; resume folds the FULL journal (§5.2's amendment). The headline the test actually pins — resume without re-spend — holds unchanged; snapshot-seeded tail-only resume is an unwired optimisation, so read this as "**Resume without re-spend**".
9. **Compaction** — after `Consolidate`, the `Map`'s per-child records are collapsed to `{index,status,digest}` (assert the compacted shape; content still fetchable by digest). — ⚠️ **Amended:** this covers a **`ModelCall`-body** `Map` with a `ContentStore` configured; the shape is `{index, status, digest, input_hash, usage}`. An **`Agent`-body `Map` is never compacted** — the explicit gap, not an untested case (§5.3's amendment).
