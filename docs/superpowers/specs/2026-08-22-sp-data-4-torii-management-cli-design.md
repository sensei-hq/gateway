...
- ~~**The DB tests' isolation guards are process-wide, not cross-process.**~~ — **PARTLY closed in SP-DATA-4.1, fully closed 2026-08-28.** 4.1 added a Postgres session-level advisory lock to the guards of *two* crates (pre-fix, two concurrent `cargo` invocations failed 7 of 8 rounds; post-fix, 16/16 clean) and this entry then claimed "`cargo nextest` is now safe". **That claim was false**, as was "both crates": **three** crates write these tables, and the third — `sensei-orchestrator`'s `executor::tests::postgres_e2e` — had **no guard at all**, having no `sqlx` dependency to build one from. An advisory lock only serializes the parties that take it, so one abstainer voided it for everyone: that crate's two `config_versions` tests raced *each other in one binary* (red 5/5 under default threads) and lost 10/10 against concurrent guarded suites. Genuinely closed by lending the ONE guard from `orchestrator_store::test_guard` to all three; 5 consecutive rounds of all three PG suites green under default threads. The `config_*` tables and
