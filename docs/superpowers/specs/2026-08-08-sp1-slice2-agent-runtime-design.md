...
4. **Prompt budget = detect-and-halt, per live turn** — budget to the **minimum `context_window`** across the resolved chain, checked **before each live turn** (the ReAct transcript grows as tool results append; memoized turns on resume are exempt); over-budget → a journaled explicit outcome that **halts loud** (`PromptOverBudget`). The summarize/select **strategy** is deferred; the "never silent truncation" **invariant** is honored now. This is a **window-fit** control only — **orthogonal** to the gateway's cost budget (`request.budget`/`BudgetGate`), which slice 2 leaves **dormant** (`budget: None`), so there is no second, silent budget layer.
...
3. **Budget** (§5) is enforced **per live turn** inside the loop (below), not once up front — the ReAct transcript grows as tool results append, so each turn re-checks fit before it spends; a **memoized** turn on resume is never re-budgeted (it already fit when first recorded).
4. Journal **`NodeStarted{node}` once**, then run the **ReAct loop**, `turn = 0, 1, 2, …` (per-turn `EffectRecorded`; a single `NodeCompleted{node}` at the final answer):
   ...
   - Else (**live** turn): first the **budget check** (§5) on the *current* `system + messages + tools` → over-budget → `PromptOverBudget{ node, turn, est, min_win }` (journal `NodeFailed`, halt, **before spend**). Then build `InferenceRequest{ capability: TextChat, chain, payload: Payload::Chat{ system: Some(system), messages, tools, .. }, allow_fallback: true, budget: None }` → `gateway.execute()`:
...
- **Budget (window-fit), per live turn:** `est = est_tokens(system) + est_tokens(messages) + est_tokens(tool schemas)` via a **documented heuristic `est_tokens(s) ≈ s.chars().count() / 4`** (explicitly *not* a real tokenizer; a conservative approximation, replaceable later). `min_win = gateway.min_context_window(chain)` (`None` → treat as "unknown", skip the check with a `tracing::warn!` — never a hard failure on missing metadata). `est > min_win` → `PromptOverBudget{ node, turn, est, min_win }`: journal `NodeFailed` + return the structured error, **before** that turn's gateway call. The check runs **before each live turn** (the transcript grows as tool results append); a **memoized** turn on resume is exempt (it fit when first recorded, and re-budgeting a completed turn would let a resume fail on already-durable work).

**Two orthogonal budgets — they don't stack or fight.** This window-fit check (tokens vs the chain's smallest context window) exists only to replace the gateway's *opaque* over-window `ProviderError` with an explicit, journaled halt (master §9.2) — it is **not** a spend control. Cost/spend is a **separate** axis owned by the gateway: `InferenceRequest.budget: Option<f64>` feeds the SP-0 `BudgetGate`. **Slice 2 passes `budget: None`, so that gate stays dormant** — there is no second, silent cost budget in play. `max_steps` (§4) is a third, distinct axis (loop length, not tokens or dollars). A run-level cost budget across turns is a later concern (master §10.2), deliberately not introduced here.
...
| `PromptOverBudget{ node, turn, est, min_win }` | a live turn's prompt exceeds the chain's smallest window (checked before each live turn; memoized turns exempt) | journal `NodeFailed`, halt before that turn's spend |
...
3. **Budget halt — pre-flight and mid-loop** — (a) a tiny `min_win` so turn 0 is already over → `PromptOverBudget{turn:0}`, **zero gateway calls**; (b) a `min_win` that fits turn 0 but not turn 1 after a tool result appends → turn 0 spends, `calc` runs, then `PromptOverBudget{turn:1}` halts **before turn 1's call** (proves the per-turn, growing-transcript check).
