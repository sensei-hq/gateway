...
- [ ] **Step 3: Implement.**
  ...
  - Stream-exhaustion terminal (the `yield StreamEvent::Error { .. }` after the loop when no candidate streamed): compute `all_gated_error(&skipped_owned, &contributions)`; if `Some(AllGated { resume_after, .. })`, yield `StreamEvent::Error { code: "all_gated".into(), message, resume_after }`; else yield the existing terminal error with `resume_after: None`. (The owned `skipped` + `contributions` must be captured into the `'static` stream closure alongside the other moved state.)
