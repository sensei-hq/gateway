...
- `RESERVED_GATE_ID = "__gate__"` exists at `crates/orchestrator-core/src/plan.rs:22` and is enforced in `feasible` at `plan.rs:147`, so an untrusted `Expand` planner cannot forge a gate node. ~~**Confirmed.**~~ **WRONG — corrected by the whole-slice review.** The check existed but walked `plan.graph.nodes` only; it did not recurse, so `feasible(Loop { body: Subgraph([Node{id:"__gate__"}]), gate: Human })` measured `Ok(())`. A hand-authored graph reached the same path (s1's `validate_dag` rule bans the `/` SEPARATOR, not a bare segment), and with a colliding node kind that COMPLETES the result was a run paused on a gate `run list-paused` omits and `run gate decide` refuses. Closed by `validate_dag` block **1c** (rejects a bare `__plan__`/`__gate__`/`__select__` id at every depth), a recursive `check_reserved_ids` in `feasible`, and an arm-level refusal to ask at a path already carrying `NodeStarted`/`NodeCompleted`. Design §5.1 carries the full record. **This is the precondition the plan said it had re-verified rather than assumed; re-verifying a claim means measuring it.**
