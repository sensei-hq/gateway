...
### SP‑3 — hierarchical executor (in progress on `develop`)
...
### Planner brainstorm — SP‑3 slice 4 (design agreed 2026‑08‑13; not yet built)
*Split into **Slice A** (core + journaled planner) and **Slice B** (selector).*
...
- **Self‑describing plan (confirmed):** `Node.plan: Option<NodePlan{label, description, needs}>`; `needs{skills,tools,agents,self_discover}` doubles as **planner‑selected activation** (the SP‑2 Q4 deferral); journaled in `PlanExpanded` so the UX/tracking see labels.
