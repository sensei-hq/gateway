...
- **⚡ SP-4 status: s1 ✅ · s2 ✅ · s5 ✅ · credential broker ✅ · workspace isolation (s3) ✅. Remaining: s4 sandbox + resource-cap KILLING — blocked on the tool-execution-model (the in-process sync `Tool::call_ctx` model can't kill/confine a runaway tool; needs a subprocess under macOS `sandbox-exec`/Linux landlock+seccomp + `setrlimit` + a kill-on-timeout supervisor). The workspace jail is the in-process spike; the subprocess sandbox is the heavy foundation, taken up when real confinement of untrusted tools is needed.**
