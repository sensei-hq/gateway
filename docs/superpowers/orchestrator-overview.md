...
- **⚡ SP-4 status: s1 ✅ · s2 ✅ · s5 ✅ · credential broker ✅. Remaining: s3 workspace isolation (premature — needs real fs tools) + s4 sandbox + cap-killing (blocked on the tool-execution-model — the executor has NO real tool I/O yet; all tools are in-process demo sinks). Both await a tool-execution-model decision.**
