//! Pure wire-format conversion between gateway types and the Anthropic
//! Messages API JSON shape.
//!
//! Everything here is a total function of its inputs — gateway
//! [`Message`]s in, Anthropic content-block request JSON out; Anthropic
//! response / SSE stream events in, [`ChatResponse`]-shaped pieces and
//! [`StreamChunk`]s out. None of it touches HTTP, auth, or adapter state
//! (the `Client`, credentials, and `RouterConfig` plumbing, plus the
//! byte-level SSE polling loop, all stay in `super`), which is what
//! makes it unit-testable without a network round-trip.
...
use kernel::types::request::{
    MediaAttachment, MediaSource, MessageContent, StreamingToolCall, ToolCall, ToolDefinition,
};
...
pub(super) fn build_messages(messages: &[Message]) -> Vec<AnthropicMessage> {
    messages
        .iter()
...
pub(super) fn build_tools(tools: &[ToolDefinition]) -> Vec<AnthropicTool> {
    tools
        .iter()
        .map(|t| AnthropicTool {
            name: t.name.clone(),
...
// ---------------------------------------------------------------------------
// Pure functions — response / stream mapping
// ---------------------------------------------------------------------------
...
pub(super) fn usage_from_anthropic(usage: &AnthropicUsage) -> TokenUsage {
    TokenUsage {
...
pub(super) fn extract_text(content: &[ContentBlock]) -> String {
    content
        .iter()
...
// ---------------------------------------------------------------------------
// SSE stream state + event parsing / streamed-tool-call accumulation
// ---------------------------------------------------------------------------
...
pub(super) fn process_stream_bytes(state: &mut AnthropicStreamState, bytes: &[u8]) {
