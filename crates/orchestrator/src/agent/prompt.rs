...
///
/// Since SP-7c that estimator is `ceil(bytes / 3) + MAX_TOKENS_PER_ATTACHMENT × attachments`,
...
/// published per-image token figure by three, which is the exact mistake SP-7c's D3 exists to
